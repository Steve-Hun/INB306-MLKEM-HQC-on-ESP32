...
static const char *TAG = "hqc_phase3";
...
static void hqc_bench_task(void *arg)
{
    bench_config_t local_cfg = {
        .phase = BENCH_PHASE3_HQC,
        .trials = 25,
    ...
    ESP_LOGI(TAG, "Phase 3: HQC-128 single-core baseline");
    ESP_LOGI(TAG, "Algorithm: HQC-128");
    ...
    // Warm-up (untimed): pay lazy inits and fill caches
    {
        ...
        rc = PQCLEAN_HQC128_CLEAN_crypto_kem_enc(ct, key_b, pk);
        if (rc != 0) { ESP_LOGE(TAG, "Warm-up encaps failed"); goto end_task; }
        vTaskDelay(1);
        ESP_LOGI(TAG, "Warm-up: decaps...");
        rc = PQCLEAN_HQC128_CLEAN_crypto_kem_dec(key_a, ct, sk);
...
void bench_hqc_start(const bench_config_t *cfg)
{
    ...
    if (cfg) {
        *cfg_copy = *cfg;
        ...
        cfg_copy->phase = BENCH_PHASE3_HQC;
        cfg_copy->trials = 25;
        cfg_copy->metric = BENCH_METRIC_MEDIAN;
    }

    const BaseType_t ok = xTaskCreatePinnedToCore(hqc_bench_task, "hqc_bench",
