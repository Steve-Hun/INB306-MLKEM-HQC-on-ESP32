...
// Include PQClean HQC-128 API
#include "api.h"

// Include mbedTLS for X25519 (using general ECP implementation)
#include "mbedtls/entropy.h"
#include "mbedtls/ctr_drbg.h"
#include "mbedtls/ecdh.h"
#include "mbedtls/ecp.h"

static const char *TAG = "hqc_hybrid_phase6";

// HQC-128 constants from api.h
...
// HQC uses deep recursion in karatsuba multiplication - needs very large stack
#ifndef BENCH_TASK_STACK_WORDS
#define BENCH_TASK_STACK_WORDS 65536  // 256 KB for HQC's recursive algorithms
#endif
...
static void hybrid_hqc_bench_task(void *arg)
{
    bench_config_t local_cfg = {
        .phase = BENCH_PHASE3_HQC,  // reuse phase 3 enum for now
        .trials = 25,
    ...
    // HQC buffers
    uint8_t hqc_pk[CRYPTO_PUBLICKEYBYTES];
    uint8_t hqc_sk[CRYPTO_SECRETKEYBYTES];
    uint8_t hqc_ct[CRYPTO_CIPHERTEXTBYTES];
    uint8_t hqc_ss_alice[CRYPTO_BYTES];
    uint8_t hqc_ss_bob[CRYPTO_BYTES];

    // X25519 using lower-level ECP API (simpler, stateless)
    ...
    ESP_LOGI(TAG, "Phase 6: HQC-128 + X25519 Hybrid (primitives only, no HKDF)");
    ESP_LOGI(TAG, "Algorithm: HQC-128 + X25519");
    ESP_LOGI(TAG, "HQC sizes: pk=%d sk=%d ct=%d shared=%d", CRYPTO_PUBLICKEYBYTES,
    ...
    const char *pers = "hqc_hybrid_bench";
    ...
    // Warm-up (untimed): pay lazy inits and fill caches
    {
        ...
        ret = PQCLEAN_HQC128_CLEAN_crypto_kem_keypair(hqc_pk, hqc_sk);
        if (ret != 0) { ESP_LOGE(TAG, "Warm-up HQC keypair failed"); goto end_task; }
        ...
        ret = PQCLEAN_HQC128_CLEAN_crypto_kem_enc(hqc_ct, hqc_ss_bob, hqc_pk);
        if (ret != 0) { ESP_LOGE(TAG, "Warm-up HQC encaps failed"); goto end_task; }
        
        ESP_LOGI(TAG, "Warm-up: HQC decaps...");
        ret = PQCLEAN_HQC128_CLEAN_crypto_kem_dec(hqc_ss_alice, hqc_ct, hqc_sk);
        if (ret != 0) { ESP_LOGE(TAG, "Warm-up HQC decaps failed"); goto end_task; }
    ...
    for (size_t i = 0; i < trials; ++i) {
        ...
        // Alice measurement 1: HQC keygen + X25519 keygen
        ...
        // Alice measurement 2: HQC decaps + X25519 DH
        start = bench_cycles_now();
        rc = PQCLEAN_HQC128_CLEAN_crypto_kem_dec(hqc_ss_alice, hqc_ct, hqc_sk);
        ...
        // Bob measurement 1: X25519 keygen + HQC encaps
        start = bench_cycles_now();
        ret = mbedtls_ecdh_gen_public(&grp, &bob_d, &bob_Q,
                                       mbedtls_ctr_drbg_random, &ctr_drbg);
        rc = PQCLEAN_HQC128_CLEAN_crypto_kem_enc(hqc_ct, hqc_ss_bob, hqc_pk);
        ...
        // Yield to allow idle and system tasks to run
...
#  endif
    ...
    bool hqc_match = (memcmp(hqc_ss_alice, hqc_ss_bob, CRYPTO_BYTES) == 0);
    bool x25519_match = (memcmp(x25519_ss_alice, x25519_ss_bob, 32) == 0);
    ESP_LOGI(TAG, "HQC shared secret match (last trial): %s", hqc_match ? "yes" : "NO");
    ESP_LOGI(TAG, "X25519 shared secret match (last trial): %s", x25519_match ? "yes" : "NO");
    
    if (hqc_match) {
        log_hex16("HQC ss[0:16] (last)", hqc_ss_alice);
    }
    if (x25519_match) {
...
void bench_hybrid_hqc_start(const bench_config_t *cfg)
{
    ...
    *task_cfg = *cfg;
    ...
    xTaskCreate(hybrid_hqc_bench_task, "hybrid_hqc_bench", BENCH_TASK_STACK_WORDS,
                task_cfg, 5, NULL);
}
